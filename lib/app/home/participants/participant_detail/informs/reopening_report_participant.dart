...
class ReopeningReportForm extends StatefulWidget {
  const ReopeningReportForm({
    super.key,
    required this.user,
  ...
  /// the user's currently-active `initialReportId`). Used for viewing
  /// historical reports.
  ...
  @override
  State<ReopeningReportForm> createState() => _ReopeningReportFormState();
}

class _ReopeningReportFormState extends State<ReopeningReportForm> {
  ...
  void initState() {
    ...
    _controllers = {
      ...
      'orientation10': TextEditingController(),
      'motiveClose': TextEditingController(),
      'motiveCloseDetail': TextEditingController(),
      'closeDate': TextEditingController(),
      'reopeningMotive': TextEditingController(),
    };
    _dateValues = {
      'arriveDate': null,
      'completedDate': null,
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasData) {
              ...
              if(widget.overrideReportId == null &&
                  userEnreda.initialReportId == null &&
                  !_isCreatingReport) {
                _isCreatingReport = true;
                Future.microtask(() async {
                  try {
                    InitialReport initialData = InitialReport(userId: userEnreda.userId);
                    if (userEnreda.socialItineraryHistory.isNotEmpty) {
                      final lastCycle = userEnreda.socialItineraryHistory.last;
                      if (lastCycle.closureReportId != null) {
                        final previousClosure = await database.getClosureReport(lastCycle.closureReportId!);
                        if (previousClosure != null) {
                          initialData = InitialReport.fromClosureReport(previousClosure);
                        }
                      }
                    }
                    String newId = await database.addInitialReport(initialData);
                    if(newId.isNotEmpty){
                      userEnreda.initialReportId = newId;
                      await database.updateUserEnredaFields(userEnreda.userId!, {'initialReportId': newId});
                    }
                  } catch (e) {
                    print("Error creating reopening report: $e");
                    if (mounted) {
                      showAlertDialog(context, title: "Error de servidor", content: "No se pudo crear el informe de reapertura. Por favor, inténtelo de nuevo más tarde.", defaultActionText: "Ok");
                    }
                  } finally {
                    if (mounted) {
            ...
            return SingleChildScrollView(
              child: Container(
                child: initialReport(context, userEnreda),
              ),
            );
          }
      ),
          ...
          children: [
            Row(
              ...
              children: [
                ...
                SpaceW8(),
                CustomTextBoldTitle(
                    title: 'Informe de reapertura de caso'.toUpperCase()),
              ],
            ),
            ...
            StreamBuilder<InitialReport?>(
                stream: _ensureReportStream(
                    database,
                    widget.overrideReportId ?? userEnreda.initialReportId),
                builder: (context, snapshot) {
                  final initialReportSaved = snapshot.data;
                  if (initialReportSaved != null) {
                    // If the saved report is missing closure data (created in a previous
                    // empty state), backfill it from the previous closure report.
                    if (initialReportSaved.motiveClose == null &&
                        userEnreda.socialItineraryHistory.isNotEmpty &&
                        !_reportDataLoaded) {
                      ...
                      Future.microtask(() async {
                        try {
                          ...
                          if (lastCycle.closureReportId != null) {
                            final previousClosure = await database.getClosureReport(lastCycle.closureReportId!);
                            if (previousClosure != null && mounted) {
                              final backfilled = InitialReport.fromClosureReport(previousClosure);
                              await database.setInitialReport(InitialReport(
                                initialReportId: initialReportSaved.initialReportId,
                                userId: backfilled.userId,
                                finished: false,
                                subsidy: backfilled.subsidy,
                                techPerson: backfilled.techPerson,
                                techPersonName: backfilled.techPersonName,
                                dniParticipant: backfilled.dniParticipant,
                                orientation1: backfilled.orientation1,
                                arriveDate: backfilled.arriveDate,
                                receptionResources: backfilled.receptionResources,
                                administrativeExternalResources: backfilled.administrativeExternalResources,
                                adminState: backfilled.adminState,
                                adminNoThrough: backfilled.adminNoThrough,
                                adminDateAsk: backfilled.adminDateAsk,
                                adminDateResolution: backfilled.adminDateResolution,
                                adminDateConcession: backfilled.adminDateConcession,
                                adminTemp: backfilled.adminTemp,
                                adminResidenceWork: backfilled.adminResidenceWork,
                                adminDateRenovation: backfilled.adminDateRenovation,
                                adminResidenceType: backfilled.adminResidenceType,
                                adminJuridicFigure: backfilled.adminJuridicFigure,
                                adminOther: backfilled.adminOther,
                                orientation2: backfilled.orientation2,
                                expirationDate: backfilled.expirationDate,
                                healthCard: backfilled.healthCard,
                                medication: backfilled.medication,
                                orientation2_1: backfilled.orientation2_1,
                                rest: backfilled.rest,
                                diagnosis: backfilled.diagnosis,
                                treatment: backfilled.treatment,
                                tracking: backfilled.tracking,
                                orientation2_2: backfilled.orientation2_2,
                                disabilityState: backfilled.disabilityState,
                                referenceProfessionalDisability: backfilled.referenceProfessionalDisability,
                                disabilityGrade: backfilled.disabilityGrade,
                                granted: backfilled.granted,
                                revisionDate: backfilled.revisionDate,
                                disabilityType: backfilled.disabilityType,
                                orientation2_3: backfilled.orientation2_3,
                                dependenceState: backfilled.dependenceState,
                                referenceProfessionalDependence: backfilled.referenceProfessionalDependence,
                                dependenceGrade: backfilled.dependenceGrade,
                                orientation2_4: backfilled.orientation2_4,
                                externalDerivation: backfilled.externalDerivation,
                                motive: backfilled.motive,
                                orientation3: backfilled.orientation3,
                                internalDerivationLegal: backfilled.internalDerivationLegal,
                                internalDerivationDate: backfilled.internalDerivationDate,
                                internalDerivationMotive: backfilled.internalDerivationMotive,
                                externalDerivationLegal: backfilled.externalDerivationLegal,
                                externalDerivationDate: backfilled.externalDerivationDate,
                                externalDerivationMotive: backfilled.externalDerivationMotive,
                                psychosocialDerivationLegal: backfilled.psychosocialDerivationLegal,
                                psychosocialDerivationDate: backfilled.psychosocialDerivationDate,
                                psychosocialDerivationMotive: backfilled.psychosocialDerivationMotive,
                                legalRepresentation: backfilled.legalRepresentation,
                                processingBag: backfilled.processingBag,
                                processingBagDate: backfilled.processingBagDate,
                                economicAmount: backfilled.economicAmount,
                                orientation4: backfilled.orientation4,
                                ownershipType: backfilled.ownershipType,
                                location: backfilled.location,
                                centerContact: backfilled.centerContact,
                                hostingObservations: backfilled.hostingObservations,
                                ownershipTypeOpen: backfilled.ownershipTypeOpen,
                                homelessnessSituation: backfilled.homelessnessSituation,
                                homelessnessSituationOpen: backfilled.homelessnessSituationOpen,
                                livingUnit: backfilled.livingUnit,
                                ownershipTypeConcrete: backfilled.ownershipTypeConcrete,
                                orientation5: backfilled.orientation5,
                                informationNetworks: backfilled.informationNetworks,
                                institutionNetworks: backfilled.institutionNetworks,
                                familyConciliation: backfilled.familyConciliation,
                                orientation7: backfilled.orientation7,
                                languages: backfilled.languages,
                                orientation9: backfilled.orientation9,
                                centerTSReference: backfilled.centerTSReference,
                                subsidyBeneficiary: backfilled.subsidyBeneficiary,
                                socialExclusionCertificate: backfilled.socialExclusionCertificate,
                                subsidyName: backfilled.subsidyName,
                                socialExclusionCertificateDate: backfilled.socialExclusionCertificateDate,
                                socialExclusionCertificateObservations: backfilled.socialExclusionCertificateObservations,
                                orientation12: backfilled.orientation12,
                                vulnerabilityOptions: backfilled.vulnerabilityOptions,
                                orientation13: backfilled.orientation13,
                                orientation13_2: backfilled.orientation13_2,
                                educationLevel: backfilled.educationLevel,
                                laborSituation: backfilled.laborSituation,
                                homologation: backfilled.homologation,
                                laborOtherConsiderations: backfilled.laborOtherConsiderations,
                                tempLabor: backfilled.tempLabor,
                                workingDayLabor: backfilled.workingDayLabor,
                                competencies: backfilled.competencies,
                                contextualization: backfilled.contextualization,
                                connexion: backfilled.connexion,
                                shortTerm: backfilled.shortTerm,
                                mediumTerm: backfilled.mediumTerm,
                                longTerm: backfilled.longTerm,
                                orientation9_5: backfilled.orientation9_5,
                                formations: backfilled.formations,
                                formationBag: backfilled.formationBag,
                                formationBagDate: backfilled.formationBagDate,
                                formationBagMotive: backfilled.formationBagMotive,
                                formationBagEconomic: backfilled.formationBagEconomic,
                                jobObtaining: backfilled.jobObtaining,
                                jobObtainDate: backfilled.jobObtainDate,
                                jobFinishDate: backfilled.jobFinishDate,
                                jobUpgrade: backfilled.jobUpgrade,
                                upgradeMotive: backfilled.upgradeMotive,
                                upgradeDate: backfilled.upgradeDate,
                                orientation9_6: backfilled.orientation9_6,
                                postLaborAccompaniment: backfilled.postLaborAccompaniment,
                                postLaborAccompanimentMotive: backfilled.postLaborAccompanimentMotive,
                                postLaborInitialDate: backfilled.postLaborInitialDate,
                                postLaborFinalDate: backfilled.postLaborFinalDate,
                                postLaborTotalDays: backfilled.postLaborTotalDays,
                                jobMaintenance: backfilled.jobMaintenance,
                                orientation10: backfilled.orientation10,
                                motiveClose: backfilled.motiveClose,
                                motiveCloseDetail: backfilled.motiveCloseDetail,
                                closeDate: backfilled.closeDate,
                              ));
                            }
                          }
                        } catch (e) {
                          print('Error backfilling reopening report: $e');
                        }
                      });
                    } else if (!_reportDataLoaded) {
                      _reportDataLoaded = true;
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        if (mounted) setState(() {});
                      });
                    }
                    return completeReopeningForm(context, initialReportSaved, userEnreda);
                  }
                  return Container();
                }
            ),
  ...
  Widget completeReopeningForm(BuildContext context, InitialReport report,
      UserEnreda user) {
    final database = Provider.of<Database>(context, listen: false);
    ...
    if (_controllers['reopeningMotive']!.text.trim().isEmpty) {
      _controllers['reopeningMotive']!.text = report.reopeningMotive ?? '';
    }
    ...
    //Section 10
    if (_controllers['orientation10']!.text.trim().isEmpty) {
      _controllers['orientation10']!.text = report.orientation10 ?? '';
    }
    if (_controllers['motiveClose']!.text.trim().isEmpty) {
      _controllers['motiveClose']!.text = report.motiveClose ?? '';
    }
    if (_controllers['motiveCloseDetail']!.text.trim().isEmpty) {
      _controllers['motiveCloseDetail']!.text = report.motiveCloseDetail ?? '';
    }
    if (_dateValues['closeDate'] == null) {
      _dateValues['closeDate'] = report.closeDate;
    }
        ...
        child: Column(
          ...
          children: [
            ...
            CustomTextFormFieldTitle(
              ...
              initialValue: _controllers['dniParticipant']!.text,
            ...
            CustomTextFormFieldTitle(
              ...
              initialValue: _controllers['orientation1']!.text,
            ...
            CustomTextFormFieldTitle(
              labelText: StringConst.INITIAL_OBSERVATIONS,
              initialValue: _controllers['orientation2']!.text,
            ...
            CustomTextFormFieldTitle(
              labelText: StringConst.INITIAL_OBSERVATIONS,
              controller: _controllers['orientation10']!,
            ...
            _controllers['motiveClose']!.text == ''
              ...
              onChanged: _finished
                  ? null
                  : (value) {
                _controllers['motiveClose']!.text = value!;
              },
              validator: (value) => value != null
                  ? null
                  : StringConst.FORM_GENERIC_ERROR,
              ...
              labelText: StringConst.CLOSURE_CLOSE_MOTIVE,
              source: StringConst.CLOSURE_MOTIVE_SELECTION,
              value: _controllers['motiveClose']!.text,
              onChanged: _finished
                  ? null
                  : (value) {
                _controllers['motiveClose']!.text = value!;
            ...
            CustomFlexRowColumn(
              ...
              childLeft: CustomTextFormFieldTitle(
                ...
                controller: _controllers['motiveCloseDetail']!,
              ...
              childRight: CustomDatePickerTitleOpen(
                labelText: StringConst.INITIAL_DATE,
                initialValue: _dateValues['closeDate'],
                onChanged: (value) {
                  _dateValues['closeDate'] = value;
                },
                enabled: !_finished,
                // validator: (value) =>
                // (value != null) ? null : StringConst.FORM_GENERIC_ERROR,
                  ...
                  child: ElevatedButton(
                      onPressed: () async {
                        database.setInitialReport(InitialReport(
                          ...
                          reopeningMotive: _controllers['reopeningMotive']!.text,
                          ...
                          orientation10: _controllers['orientation10']!.text,
                          motiveClose: _controllers['motiveClose']!.text,
                                    ...
                                    onPressed: () async {
                                      database.setInitialReport(
                                          InitialReport(
                                            ...
                                            psychosocialDerivationMotive: _controllers['psychosocialDerivationMotive']!.text,
                                            legalRepresentation:
                                            _controllers['legalRepresentation']!.text,
                                            processingBag: _controllers['processingBag']!.text,
                                            processingBagDate: _dateValues['processingBagDate'],
                                            economicAmount: _controllers['economicAmount']!.text,
                                            orientation4: _controllers['orientation4']!.text,
                                            ...
                                            orientation5: _controllers['orientation5']!.text,
                                            informationNetworks:
                                            _controllers['informationNetworks']!.text,
                                            institutionNetworks:
                                            _controllers['institutionNetworks']!.text,
                                            familyConciliation:
                                            _controllers['familyConciliation']!.text,
                                            orientation7: _controllers['orientation7']!.text,
                                            languages: _languagesInit.notifier.value,
                                            orientation9: _controllers['orientation9']!.text,
                                            centerTSReference:
                                            ...
                                            competencies: _controllers['competencies']!.text,
                                            contextualization:
                                            _controllers['contextualization']!.text,
                                            connexion: _controllers['connexion']!.text,
                                            shortTerm: _controllers['shortTerm']!.text,
                                            mediumTerm: _controllers['mediumTerm']!.text,
                                            longTerm: _controllers['longTerm']!.text,
